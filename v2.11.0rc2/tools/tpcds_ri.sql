...
--- changed cr_i into cr_i_rt
alter table catalog_returns add constraint cr_i_rt foreign key  (cr_returned_time_sk) references time_dim (t_time_sk);
...
--- changed wr_ret_cd into wr_ret_cd_1
alter table web_returns add constraint wr_ret_cd_1 foreign key  (wr_returning_hdemo_sk) references household_demographics (hd_demo_sk);
...
alter table web_sales add constraint ws_b_cd_1 foreign key  (ws_bill_hdemo_sk) references household_demographics (hd_demo_sk);
